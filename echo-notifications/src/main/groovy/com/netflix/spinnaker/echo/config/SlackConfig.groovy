...
//    Endpoint slackEndpoint = newFixedEndpoint(config.baseUrl)
    ...
    def slackClient = new Retrofit.Builder()
      .baseUrl(Objects.requireNonNull(HttpUrl.parse(config.baseUrl)))
      .client(retrofitClient.newBuilder()
        .addInterceptor(new RetrofitResponseInterceptor())
        .addInterceptor(new HttpLoggingInterceptor(new Slf4jRetrofitLogger(SlackClient.class)).setLevel(retrofitLogLevel))
        .build())
      .addConverterFactory(JacksonConverterFactory.create())
      .build().create(SlackClient.class);
